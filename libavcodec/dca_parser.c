                            ...
                            int *profile)
{
        ...
        *bit_rate = 1;   // set default 1
        ...
        if (asset->extension_mask & DCA_EXSS_CORE) {
            ...
            *duration = 512 * (exss_parse->exss_fr_duration_code+1) * exss_parse->ref_clock_code;
            // When extension_mask is DCA_EXSS_CORE, profile can be DTS/DTS_HD_MA/DTS_EXPRESS.
            *profile = FF_PROFILE_DTS; // don't care about it.
            return 0;
        }
    ...
    *channel_count = ff_dca_channels[h.audio_mode] + !!h.lfe_present;
    *bit_rate = ff_dca_bit_rates[h.br_code];
                     ...
                     const uint8_t *buf, int buf_size)
{
    ...
    ParseContext *pc = &pc1->pc;
    DCAExssAsset *asset = &pc1->exss.assets[0];
    int next, duration, sample_rate, channel_count, bit_rate;
    next = duration = sample_rate = channel_count = bit_rate = 0;
    ...
    if (!dca_parse_params(pc1, buf, buf_size, &duration, &sample_rate, &channel_count, &bit_rate, &avctx->profile)) {
        if (!avctx->sample_rate)
            avctx->sample_rate = sample_rate;

        if (!avctx->channels && asset->nchannels_total)
            avctx->channels = asset->nchannels_total;
        else if (!avctx->channels && channel_count)
            avctx->channels = channel_count;

        if (!avctx->bit_rate && bit_rate)
