...
static int mov_read_pssh(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    uint8_t **key_ids;
    AVStream *st;
    uint8_t *side_data, *extra_data, *old_side_data = NULL;
    ...
    memcpy(info->system_id, &pssh->uuid, 16);
    ...
    if (version > 0) {
        ...
        for (unsigned int num = 0; num < kid_count && !pb->eof_reached; num++) {
            unsigned int min_kid_count = FFMIN(FFMAX(num + 1, 1024), kid_count);
            ...
            info->key_ids[num] = av_mallocz(16);
            if (!info->key_ids[num]) {
                ret = AVERROR(ENOMEM);
                goto finish;
            }
            info->num_key_ids = num + 1;

            if ((ret = ffio_read_size(pb, info->key_ids[num], 16)) < 0) {
...
static int mov_read_dmlp(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    //because if mlpparser is removed .
...
#ifdef AMFFMPEG
            int sample_current;
            sample_current = mov_seek_stream(s, st, timestamp, flags);
            if (sample_current >= 0)
                st->internal->skip_samples = mov_get_skip_samples(st, sample_current);
            else {
