                                  ...
                                  uint32_t stream_type, uint32_t prog_reg_desc)
{
    ...
    if (st->codec->codec_id == AV_CODEC_ID_NONE && pes->stream_type == 0x82) {
        ...
        sub_st->priv_data = sub_pes;
        sub_st->codec->codec_type = AVMEDIA_TYPE_AUDIO;
        sub_st->codec->codec_id   = AV_CODEC_ID_DTS;
        sub_st->need_parsing = AVSTREAM_PARSE_FULL;
        sub_pes->sub_st = pes->sub_st = sub_st;
#endif
    }
                              ...
                              MpegTSContext *ts)
{
    ...
    case 0x7f: /* DVB extension descriptor */
        ...
        if (ext_desc_tag == 0x15) {
            ...
            AVPacketSideData* sidedate = st->side_data;
            AVPacketSideData* ac4_sidedate = NULL;
            if (sidedate) {
                for (int i = 0; i < st->nb_side_data; i++) {
                    if (sidedate[i].type == AV_PKT_AUDIO_PRESELECTION_DATA) {
                        ac4_sidedate = sidedate+i;
                        break;
                    }
                }
            }
            ...
            if (ac4_sidedate) {
                av_free(ac4_sidedate->data);
            } else {
                st->side_data = av_realloc(st->side_data, (st->nb_side_data+1)*sizeof(AVPacketSideData));
                ac4_sidedate = st->side_data + st->nb_side_data;
                st->nb_side_data++;
            }
            if (ac4_sidedate) {
                ac4_sidedate->type = AV_PKT_AUDIO_PRESELECTION_DATA;
                ac4_sidedate->size = desc_end-(*pp);
                ac4_sidedate->data = av_memdup(*pp,ac4_sidedate->size);
            }
        } else
#endif
...
#ifdef AMFFMPEG
                (stream_type == 0x83 || stream_type == 0x81) && pes->sub_st) {
#else
...
#ifdef AMFFMPEG
            case 0x50:
                //DVB spec, 0X50 is component descriptor
